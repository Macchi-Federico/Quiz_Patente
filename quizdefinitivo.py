...
class AppQuiz:
    def __init__(self, master):
        ...
        # Frame principale
        frame_principale = tk.Frame(self.master, bg="#e5d1b0")
        frame_principale.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        # Frame superiore per info
        frame_superiore = tk.Frame(frame_principale, bg="#e5d1b0")
        frame_superiore.pack(fill=tk.X)
        
        # Timer e counter delle domande con stile migliorato
        self.frame_timer = tk.Frame(frame_superiore, bg="#e5d1b0", bd=1, relief=tk.RAISED)
        ...
        self.style.configure("TProgressbar",
                             troughcolor="#e5d1b0",  # Colore dello sfondo della barra
                             background="#ffc966")  # Colore della barra per il punteggio
        ...
        frame_contenuto = tk.Frame(frame_principale, bg="#e5d1b0", pady=10  )
        ...
        self.frame_immagine = tk.Frame(frame_contenuto, bg="#f7f1e7", bd=1, relief=tk.SUNKEN)
        self.frame_immagine.pack(pady=20)
        
        self.etichetta_immagine = tk.Label(self.frame_immagine, bg="#f7f1e7")
        self.etichetta_immagine.pack(padx=5, pady=5)
        
        # Frame per i bottoni delle risposte
        frame_risposte = tk.Frame(frame_principale, bg="#e5d1b0")
        ...
        frame_navigazione = tk.Frame(frame_principale, bg="#e5d1b0")
    ...
    def carica_immagine(self, percorso_immagine):
        if percorso_immagine:
            try:
                ...
                img = img.resize((250, 250))  # Cambia la dimensione come necessario
    ...
    def termina_quiz(self):
        if self.punteggio >= 27:
            messagebox.showinfo("Quiz Terminato", f"Hai totalizzato {self.punteggio} punti su {len(domande)}. Sei PASSATO!")
        else:
